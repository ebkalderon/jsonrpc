...
pub type FutureResponse = Box<dyn Future<Output = Option<Response>> + Send + Unpin>;

/// A type representing middleware or RPC call output.
pub type FutureOutput = Box<dyn Future<Output = Option<Output>> + Send + Unpin>;
...
impl<T: Metadata, S: Middleware<T>> MetaIoHandler<T, S> {
	...
	pub fn handle_request_sync(&self, request: &str, meta: T) -> Option<String> {
		executor::block_on(self.handle_request(request, meta))
	}
...
mod tests {
	...
	fn test_io_handler() {
		...
		io.add_method("say_hello", |_| future::ok(Value::String("hello".to_string())));
	...
	fn test_extending_by_multiple_delegates() {
		...
		struct Test;
		impl Test {
			fn abc(&self, _p: crate::Params) -> Ready<Result<Value, crate::Error>> {
				future::ok(5.into())
			}
		}
